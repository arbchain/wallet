...
const createUserWallet = async function (password, orionKey) {
    if (password == null || password.length === 0 || orionKey == null || orionKey.length === 0)
        return false
    try {
        const entropy = "CreateEntropyPasswordWalletErrorOrionKeyLengthFalseTruePackageJson"
...
const walletLogin = async function (password) {
    if (password == null || password.length === 0)
        return null
    try {
        const userAccounts = await wallet.getAllAccounts(password)
